  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!weight) {
        ...
        if (moment.tz(weight.date, format, zone).isBefore(moment.tz(format, zone))) {
          const nWeight = await Weight.findOneAndUpdate({ _id: weight.id }, { cal_progress: 0, water: 0.0, exercise: 0 })
          nWeight?.save()
